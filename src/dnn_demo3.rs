...
fn main() -> Result<()> {
    ...
    let mut opt: AdamW = AdamW::new(varmap.all_vars(), ParamsAdamW::default())?;

    let (x, y) = dataset(IN_SIZE, LABELS, N_SAMPLES, &dev)?;
    let mut total_val_losses = Vec::<f32>::with_capacity(EPOCHS);
    let losses = dnn.train(&mut opt, &x, &y, 0.8, EPOCHS)?;
